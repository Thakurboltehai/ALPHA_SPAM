...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"•[ 🍹𝐓 ʜ 𝜶֟፝ ᴋ ᴜ ʀ 🍹 ]•")
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await altron.edit(f"[🍹] 𝐓 ʜ 𝜶֟፝ ᴋ ᴜ ʀ ᴘαᴘα ɪѕ нєʀє\n[🏓] αвє αв тєʀα куα нσgα\n[⚡] кιѕкι ᴄнυ∂αι кαʀиι нαι\n\n➜ `{mp} ms`")
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"»🍃 𝐓 ʜ 𝜶֟፝ ᴋ ᴜ ʀ ᴋᴀ єк σʀ иєω вєтα αᴅᴅ нσ gуα 🍃")
        target = ""
